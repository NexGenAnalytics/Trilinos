...
void CDR_Test(const Comm& comm, Teuchos::FancyOStream &out, bool& success){
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    // Output finest temporal solution for plotting
    // This only works for ONE MPI process
    if ((n == nTimeStepSizes-1) && (comm->getSize() == 1)) {
      std::ofstream ftmp("Tempus_BackwardEuler_CDR.dat");
      ftmp << "TITLE=\"Backward Euler Solution to CDR\"\n"
           << "VARIABLES=\"z\",\"T\"\n";
      const double dx = std::fabs(left_end-right_end) /
...
TEUCHOS_UNIT_TEST(BackwardEuler, CDR_Tpetra)
{
  ...
  CDR_Test<SC, Tempus_Test::CDR_Model_Tpetra<SC, LO, GO, Node>>(comm, out, success);
}
